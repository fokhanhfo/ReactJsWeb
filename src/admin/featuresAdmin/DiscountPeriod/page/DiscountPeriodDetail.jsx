...
import { useGetIdDiscountPeriodQuery } from 'hookApi/discountPeriodApi';
import { formatDateTime } from 'utils';
import ProductDiscountPeriod from '../components/ProductDiscountPeriod';

DiscountPeriodDetail.propTypes = {};
...
function DiscountPeriodDetail(props) {
  const [value, setValue] = React.useState(0);
  const { discountPeriodId } = useParams();
  const { data, error, isLoading, refetch } = useGetIdDiscountPeriodQuery(discountPeriodId);
  ...
  const copyToClipboard = () => {
    if (!discount?.discountPeriodCode) return;
    navigator.clipboard.writeText(discount.discountPeriodCode);
    setCopied(true);
    ...
    <Paper>
      ...
      >
        ...
        <Box>
          ...
          >
            Mã Giảm Giá: {discount?.discountPeriodCode}
          </Typography>
          <Typography
            variant="subtitle1"
            sx={{
      ...
      <Box sx={{ width: '100%' }}>
              ...
              <Grid item xs={12} md={6}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                  ...
                  <Box>
                    ...
                    <Typography variant="body2">{discount.minPercentageValue}%</Typography>
                  </Box>
                </Box>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                  ...
                  <Box>
                    ...
                    <Typography variant="body2">{discount.maxPercentageValue}%</Typography>
                  </Box>
                </Box>
              </Grid>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                  ...
                  <Box>
                    ...
                    <Typography variant="body2">
                      {formatDateTime(discount.startTime)} → {formatDateTime(discount.endTime)}
                    </Typography>
                  </Box>
                </Box>
        ...
        <CustomTabPanel value={value} index={1}>
          {discount.productDiscountPeriods && (
            <ProductDiscountPeriod
              onSubmit={onSubmit}
              idDiscount={discount.id}
              discountsProduct={discount.productDiscountPeriods}
...
export default DiscountPeriodDetail;
